...
class GPRcopy: 
    ...
    def hyper_parameter_optimisation(self, hyper_params, warp_params = None):
        #x0 = np.array([np.log(hyper_params[0])] + [np.log(self.a0), np.log(self.b0), np.log(self.c0 + 0.03)]*I)
        x0 = np.array([np.log(hyper_params[1])])
        params = np.exp(x0)
        self.set_hyper_params([hyper_params[0],params[0]], self.sigma_n)
        nlml = -self.log_marginal_likelihood()
        res = minimize(nlml, params, method='powell',
                    options={'xatol': 1e-10, 'disp': True})
        optimal_params = np.exp(res)
