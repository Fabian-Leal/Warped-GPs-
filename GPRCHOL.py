...
class GaussianProcessRegressionLengthscaleVarianceCholesky:
    ...
    def cov_func(self, x_1, x_2):
        ...
        if self.cov_function_name == "Matern":
            if self.nu == 5 / 2:
                K = self.hyper_params[0] * (1 + np.sqrt(5) * d / self.hyper_params[1]
                                            + 5 * d ** 2 / (3 * self.hyper_params[1] ** 2)) \
    ...
    def log_marginal_likelihood(self):
        alpha = cho_solve(self.L, self.f)
        lml1 = -0.5 * np.dot(np.transpose(self.f), alpha)
        lml2 = -0.5 * np.sum(np.log(np.diag(self.L[0]))) - np.log(np.diag(self.L[0])).shape[0] * np.log(2)
        lml3 = -0.5 * self.x_train.shape[0] * np.log(2 * np.pi)
    ...
    def fit(self):
        if self.warp_params is None:
            ...
            result = minimize(obj_func, np.log([self.hyper_params[1]]), method='Powell')
            new_hyper_params = np.exp(result.x)
            self.set_hyper_params([self.hyper_params[0], new_hyper_params[0]], self.sigma_n)
        else:
            def obj_func(params):
                params = np.exp(params)
                self.set_hyper_params([self.hyper_params[0], params[0]], self.sigma_n,
                                      [[self.gamma[0]], [params[1]], [self.gamma[2]]])
                return -self.log_marginal_likelihood()

            result = minimize(obj_func, np.log([self.hyper_params[1], self.gamma[1]]), method='Powell')
            new_hyper_params = np.exp(result.x)
            self.set_hyper_params([self.hyper_params[0], new_hyper_params[0]], self.sigma_n,
                                  [[self.gamma[0]], [new_hyper_params[1]], [self.gamma[2]]])
            
        return self.optimal_params



class GaussianProcessRegressionLengthscaleCholesky:
    def __init__(self, cov_function_name, x_train, y_train, hyper_params, sigma_n, warp_params=None, nu=None):
        self.cov_function_name = cov_function_name
        self.x_train = x_train
        self.y_train = y_train
