...
class GPRCHOL:
    ...
    def predict(self, x_star):
        ...
        f_mean = np.dot(k_star.T, self.alpha)
        V = cho_solve(self.L, k_star)
        f_std = self.cov_func(x_star, x_star) - np.dot(k_star.T, V)
    ...
    def log_marginal_likelihood(self):
        L = cho_factor(self.K + np.eye(self.K.shape[0]) * (self.sigma_n ** 2))
        alpha = cho_solve(L, self.y_train)
        lml1 = -0.5 * np.dot(np.dot(np.transpose(self.f), alpha), self.f)
        lml2 = -np.sum(np.log(np.diag(L[0])))  # Log determinant using Cholesky decomposition
        lml3 = -0.5 * self.x_train.shape[0] * np.log(2 * np.pi)
        lml4 = np.sum(np.log(self.df_dy))
        lml = lml1 + lml2 + lml3 + lml4
        return lml
